...
//	float len = length(g_vLightPos - vWorldPos) / g_fFarClip;
	...
	CalculateDistFromLightToVertex( proj_mode, vWorldPos.xyz );
...
//	return float4( float3(1,1,1) * variance, 1 ); // debug
//	return float4( float3(1,1,1) * m_d, 1 ); // debug
//	return float4( float3(1,1,1) * (m_d*m_d), 1 ); // debug
