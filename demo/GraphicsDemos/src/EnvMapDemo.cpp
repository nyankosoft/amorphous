...
{
	m_pEnvMapDemo->RenderNonEnvMappedMeshes();
}
...
int EnvMapDemo::Init()
{
	...
	{
		loaded = m_EnvMappedMeshShader.Load( "./shaders/EnvMapDemo.fx" );
	}
...
//		return -1;
	...
	m_pCubeMapSceneRenderer.reset( new CubeMapDemoRenderer(this) ); 
...
void EnvMapDemo::Update( float dt )
{
}


void EnvMapDemo::RenderBase()
{
	PROFILE_FUNCTION();
...
void EnvMapDemo::RenderNonEnvMappedMeshes()
{
	ShaderManager *pShaderManager = m_EnvMappedMeshShader.GetShaderManager();
//	if( !pShaderManager )
//		return;
...
void EnvMapDemo::HandleInput( const InputData& input )
{
...
void EnvMapDemo::UpdateViewTransform( const Matrix44& matView )
{
//	m_pCubeMapManager->UpdateViewTransform( matView );
}


void EnvMapDemo::UpdateProjectionTransform( const Matrix44& matProj )
{
