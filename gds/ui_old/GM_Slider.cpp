...
: CGM_Control( pDialog, pDesc )
{
	...
	{	// rect is not specified - use the default bounding box
		...
		m_BoundingRectElement.m_RectColor.m_aColor[GM_STATE_NORMAL].SetRGBA(0.7f, 0.7f, 0.7f, 0.8f);
		m_BoundingRectElement.m_RectColor.m_aColor[GM_STATE_PRESSED].SetRGBA(0.7f, 0.7f, 0.7f, 0.8f);
		m_BoundingRectElement.m_RectColor.m_aColor[GM_STATE_MOUSEOVER].SetRGBA(0.7f, 0.7f, 0.7f, 0.8f);
	}
	...
	{	// rect is not specified - set the default button
		m_ButtonElement.m_RectColor.m_aColor[GM_STATE_NORMAL].SetRGBA(0.7f, 0.7f, 0.7f, 1.0f);
		m_ButtonElement.m_RectColor.m_aColor[GM_STATE_PRESSED].SetRGBA(1.0f, 1.0f, 1.0f, 1.0f);
		m_ButtonElement.m_RectColor.m_aColor[GM_STATE_MOUSEOVER].SetRGBA(0.8f, 0.8f, 0.8f, 1.0f);
	}
...
bool CGM_Slider::HandleMouseInput( SInputData& input )
{
...
//				m_iDragY = pt.y;
			...
			{	// pressed inside the slider control
				if( m_iButtonX + m_fPosX < pt.x )
				{
					SetValueInternal( m_iValue + 1, true );
					return true;
				}
				
				if( pt.x < m_fPosX + m_iButtonX )
				{
					SetValueInternal( m_iValue - 1, true );
					return true;
				}
...
//				m_pDialog->ClearFocus();
	...
	case GIC_MOUSE_AXIS_Y:
		if( m_bPressed )
		{
			SetValueInternal( ValueFromPos( m_fPosX + pt.x + m_iDragOffset ), true );
			return true;
		}
		break;
...
void CGM_Slider::SetValueInternal( int iValue, bool bFromInput )
{
    ...
    m_pDialog->SendEvent( GM_EVENT_SLIDER_VALUE_CHANGED, bFromInput, this );
}
...
void CGM_Slider::Render( float fElapsedTime )
{
    int iOffsetX = 0;
    int iOffsetY = 0;
...
/*	if( m_bVisible == false )
	...
	float fBlendRate = ( iState == GM_STATE_PRESSED  ) ? 0.0f : 0.8f;

    // Blend current color
	m_BoundingRectElement.m_RectColor.Blend( iState, fElapsedTime, fBlendRate /*0.2f*/ );
	...
	m_ButtonElement.m_RectColor.Blend( iState, fElapsedTime, fBlendRate /*0.2f*/ );
