...
struct SBVHFrameData
{
	...
	vector<float> m_vecfChannelValue;

	inline int GetNumChannles() const { return m_vecfChannelValue.size(); }
...
protected:
	...
	string m_strBVHFilename;

	/// stores motion data
	vector<SBVHFrameData> m_vecFrame;

	/// stores the channel type info
	vector<int> m_vecChannelType;

	/// used to temporarily hold interpolated frame values
	vector<float> m_vecfInterpolatedFrame;
...
public:
	...
	virtual bool LoadBVHFile( const string& filename );

	const string& GetBVHFileName() const { return m_strBVHFilename; }
	...
	void GetGlobalPositionsAtFrame( int iFrame, vector<Vector3>& rvecDestGlobalPositions );
	...
	vector<SBVHFrameData>& GetFrameData() { return m_vecFrame; }
	...
	void MoveOffset( int iFrame, D3DXVECTOR3 vOffset,
