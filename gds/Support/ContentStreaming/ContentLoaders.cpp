...
//--------------------------------------------------------------------------------------
CTextureLoader::CTextureLoader( const char* szFileName, CPackedFile* pPackedFile ) :
m_pData(NULL),
m_cBytes(0),
m_pPackedFile(pPackedFile)
{
//	StringCchCopyA( m_szFileName, MAX_PATH, szFileName );
...
BOOL CTextureProcessor::PopulateTexture()
{
	...
	void* pTexData = (BYTE*)m_pData + sizeof(DWORD) + sizeof(DDSURFACEDESC2_32BIT);
...
/*	if( LDT_D3D10 == m_Device.Type )
	{
		m_pRealRV10 = m_pResourceReuseCache->GetFreeTexture10( Width, Height, MipLevels, (UINT)Format );
	...
	{
		m_pRealTexture9 = m_pResourceReuseCache->GetFreeTexture9( Width, Height, MipLevels, (UINT)Format, &m_pStagingTexture9 );
		...
		else
		{
			...
			for( UINT i=0; i<m_iNumLockedPtrs; i++ )
			{
				hr = m_pStagingTexture9->LockRect( i, &m_pLockedRects[i], NULL, 0 );
				if(FAILED(hr))
				{
					m_iNumLockedPtrs = 0;
...
/*	if( LDT_D3D10 == m_Device.Type )
	...
	else*/ if( LDT_D3D9 == m_Device.Type )
	{
		...
		for( UINT i=0; i<m_iNumLockedPtrs; i++ )
		{
			hr = m_pStagingTexture9->UnlockRect( i );
			if(FAILED(hr))
			{
				return hr;
			}
		...
		// UpdateTexture
		m_Device.pDev9->UpdateTexture( m_pStagingTexture9, m_pRealTexture9 );
...
//	}
	...
	if( g_bUseMemoryMappedIO && g_bWarmIOCache )
	{
		WarmIOCache( (BYTE*)pData, cBytes );
	}
...
CVertexBufferLoader::~CVertexBufferLoader() { Destroy(); }
HRESULT WINAPI CVertexBufferLoader::Decompress( void** ppData, SIZE_T* pcBytes ) { *ppData = NULL; *pcBytes = 0; return S_OK; }
HRESULT WINAPI CVertexBufferLoader::Destroy() { return S_OK; }
HRESULT WINAPI CVertexBufferLoader::Load() { return S_OK; }
...
/*	if( LDT_D3D10 == m_Device.Type )
	...
	else*/ if( LDT_D3D9 == m_Device.Type )
	{
		...
		{
			// Lock
			hr = m_pRealBuffer9->Lock( 0, 0, (void**)&m_pLockedData, D3DLOCK_DISCARD );
			if(FAILED(hr)) 
			{
				m_pLockedData = NULL;
				return hr;
...
CIndexBufferLoader::~CIndexBufferLoader() { Destroy(); }
HRESULT WINAPI CIndexBufferLoader::Decompress( void** ppData, SIZE_T* pcBytes ) { *ppData = NULL; *pcBytes = 0; return S_OK; }
HRESULT WINAPI CIndexBufferLoader::Destroy() { return S_OK; }
HRESULT WINAPI CIndexBufferLoader::Load() { return S_OK; }
...
// SDKMesh
//--------------------------------------------------------------------------------------
CSDKMeshLoader::CSDKMeshLoader( CDXUTSDKMesh* pMesh, ID3D10Device* pDev10, LPCTSTR szFileName, bool bOptimize, bool bCreateAdjacencyIndices, SDKMESH_CALLBACKS10* pLoaderCallbacks ) :
m_pMesh( pMesh ),
m_Device( pDev10 ),
...
//	StringCchCopy( m_szFileName, MAX_PATH, szFileName );
	strncpy( m_szFileName, MAX_PATH, szFileName );
}

//--------------------------------------------------------------------------------------
CSDKMeshLoader::CSDKMeshLoader( CDXUTSDKMesh* pMesh, IDirect3DDevice9* pDev9, LPCTSTR szFileName, bool bOptimize, bool bCreateAdjacencyIndices, SDKMESH_CALLBACKS9* pLoaderCallbacks ) :
m_pMesh( pMesh ),
m_Device( pDev9 ),
m_bOptimize(bOptimize),
m_bCreateAdjacencyIndices(bCreateAdjacencyIndices),
...
HRESULT WINAPI CSDKMeshLoader::Load()
{
	...
	if( m_pMesh )
	{
		if( LDT_D3D10 == m_Device.Type )
		{
			hr = m_pMesh->Create( m_Device.pDev10, m_szFileName, m_bOptimize, m_bCreateAdjacencyIndices, m_pLoaderCallbacks10 );
		}
		else if (LDT_D3D9 == m_Device.Type )
		{
			hr = m_pMesh->Create( m_Device.pDev9, m_szFileName, m_bOptimize, m_bCreateAdjacencyIndices, m_pLoaderCallbacks9 );
		}
	}
...
CSDKMeshProcessor::~CSDKMeshProcessor() { Destroy(); }
HRESULT WINAPI CSDKMeshProcessor::LockDeviceObject( ) { return S_OK; }
HRESULT WINAPI CSDKMeshProcessor::UnLockDeviceObject( ) { return S_OK; }
HRESULT WINAPI CSDKMeshProcessor::Destroy() { return S_OK; }
HRESULT WINAPI CSDKMeshProcessor::Process( void* pData, SIZE_T cBytes ) { return S_OK; }
HRESULT WINAPI CSDKMeshProcessor::CopyToResource( ) { return S_OK; }
