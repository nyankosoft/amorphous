...
bool g_bUseMemoryMappedIO = false;
bool g_bWarmIOCache = false;
...
//--------------------------------------------------------------------------------------
CAsyncLoader::CAsyncLoader( UINT NumProcessingThreads ) :
m_bDone(false),
...
//--------------------------------------------------------------------------------------
HRESULT CAsyncLoader::AddWorkItem( IDataLoader* pDataLoader, IDataProcessor* pDataProcessor, HRESULT* pHResult, void** ppDeviceObject )
{
...
//	WCHAR szMessage[MAX_PATH];
	...
	while( !m_bDone )
	{
		...
		EnterCriticalSection( &m_csIOQueue );
		ResourceRequest = m_IOQueue.at( 0 );
//		m_IOQueue.Remove( 0 );
		...
		if( !ResourceRequest.bCopy )
		{
			if( !ResourceRequest.bError )
			{
				...
				if( FAILED(hr) )
				{
//					StringCchPrintf( szMessage, MAX_PATH, L"FileIO Error: hr = %x\n", hr );
...
//					OutputDebugString( L"Failed to Copy Data to Device Object\n" );
					assert( !"Failed to Copy Data to Device Object\n" );
...
//	WCHAR szMessage[MAX_PATH];
	...
	while( !m_bDone )
	{
		...
		EnterCriticalSection( &m_csProcessQueue );
		RESOURCE_REQUEST ResourceRequest = m_ProcessQueue.at( 0 );
//		m_ProcessQueue.Remove( 0 );
		...
		if( FAILED(hr) )
		{
//			StringCchPrintf( szMessage, MAX_PATH, L"Processing Thread Error: hr = %x\n", hr );
//			OutputDebugString(szMessage);
			assert( !"Processing Thread Error\n" );
...
bool CAsyncLoader::InitAsyncLoadingThreadObjects( UINT NumProcessingThreads )
{
	...
	{
		m_phProcessThreads[i] = (HANDLE)_beginthreadex( NULL, 0, _ProcessingThreadProc, (LPVOID)this, CREATE_SUSPENDED, NULL );
...
void CAsyncLoader::ProcessDeviceWorkItems( UINT CurrentNumResourcesToService, BOOL bRetryLoads )
{
	...
	EnterCriticalSection( &m_csRenderThreadQueue );
		size_t numJobs = m_RenderThreadQueue.size();
	LeaveCriticalSection( &m_csRenderThreadQueue );

	for(size_t i=0; i<numJobs && i < CurrentNumResourcesToService; i++ )
	{
		EnterCriticalSection( &m_csRenderThreadQueue );
			RESOURCE_REQUEST ResourceRequest = m_RenderThreadQueue.at(0);
//			m_RenderThreadQueue.Remove( 0 );
		...
		else
		{
			...
			SafeDelete( ResourceRequest.pDataLoader );
			SafeDelete( ResourceRequest.pDataProcessor );
