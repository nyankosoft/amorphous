...
//	float4 DirToCamera : COLOR1	// store direction to the camera for each vertex
};
    ...
    float2 Tex2 : TEXCOORD2 )
{
	...
	float3 V = normalize(g_vCamPos - vWorldPos);                // view direction (world space)
...
//-----------------------------------------------------------------------------
VS_OUTPUT VS_Main ( float4 vPosition  : POSITION,
                    ...
                    float2 tc         : TEXCOORD0)
{
   ...
   Out.Pos = mul( vPosition, WorldViewProj );

   float3 P = mul( vPosition, WorldView );           //position in view space
// float3 N = mul( vNormal, (float3x3)WorldViewIT ); //normal in view space
   float3 N = mul( vNormal, (float3x3)WorldView ); //normal in view space
...
{
	return tex2D(Sampler0, Tex0) * Diff;
}
...
technique MultiPassShader
{
    ...
    pass P2
    {
        // entity
        VertexShader = NULL;
		PixelShader  = compile ps_2_0 SingleTexPS();
...
//      PixelShader  = NULL;
    ...
    {
        // pseudo opt-cam
        VertexShader = compile vs_1_1 OC_TestVS();
        PixelShader  = NULL;						// compile ps_2_0 SingleTexPS();
    }

    pass P4
    {
        // pseudo night vision for static geometry
        VertexShader = compile vs_1_1 VS_StaticGeometry();
        PixelShader  = compile ps_2_0 PS_PseudoNightVisionSG();
    }
}
...
//    Out.ColorSpec = I_s * k_s * pow(max(0, dot(R, V)), n/4);   // specular
